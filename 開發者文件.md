...
4.  **轉換**:
  ...
  ### v1.2 更新 (2026/01/19) - 穩定性與格式優化
1.  **Word 分頁機制重構**：
    *   **問題**：Pandoc 轉換 Word 時忽略 `\newpage`。
    *   **解法**：在 `textToMarkdown.js` 插入通用標記 `<!-- PAGE_BREAK -->`，並在 `server.js` 中於寫入暫存檔前將其替換為 Word OpenXML 強制分頁碼：
        ```javascript
        const wordPageBreak = '\n```{=openxml}\n<w:p><w:r><w:br w:type="page"/></w:r></w:p>\n```\n';
        ```
2.  **表格識別增強**：
    *   **自動偵測**：放寬偵測標準，納入「連續短行」特徵。
    *   **強制標記 (Explicit Markers)**：新增 `【表格開始】` 與 `【表格結束】` 語法，允許使用者強制指定表格區塊，繞過自動偵測邏輯。
3.  **AI 請求穩定性強化**：
    *   **Retry 機制**：針對 `429 Too Many Requests` 錯誤實作指數退避 (Exponential Backoff)，等待時間設為 10s, 20s, 30s，確保在免費額度限制下仍能完成轉換。
    *   **模型維持**：測試後發現 `gemini-1.5-flash` API 尚未對所有帳號開放 (404 Error)，故維持使用 `gemini-2.0-flash-exp` 並搭配上述重試機制。
